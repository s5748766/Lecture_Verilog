...
// 3x3 dilation (replicate border)
static void dilate3x3(const uint8_t* src, uint8_t* dst){
    for(int y=0;y<H;y++){
        for(int x=0;x<W;x++){
            int m = 0;
            for(int dy=-1; dy<=1; dy++){
                for(int dx=-1; dx<=1; dx++){
                    int v = src[IDX(x+dx,y+dy)];
                    if(v>m) m=v;
                }
            }
            dst[y*W+x] = (uint8_t)m;
...
// 3x3 erosion (replicate border)
static void erode3x3(const uint8_t* src, uint8_t* dst){
    for(int y=0;y<H;y++){
        for(int x=0;x<W;x++){
            int m = 255;
            for(int dy=-1; dy<=1; dy++){
...
static void morph_gradient(const uint8_t* src, uint8_t* dst){
    uint8_t* dil = (uint8_t*)malloc(NPIX);
    uint8_t* ero = (uint8_t*)malloc(NPIX);
    if(!dil || !ero){ printf("OOM\n"); exit(1); }
    dilate3x3(src,dil);
    erode3x3(src,ero);
    for(int i=0;i<NPIX;i++){
        int v = (int)dil[i] - (int)ero[i];
...
int main(int argc, char** argv){
    ...
    morph_gradient(gray,out);

    char gpath[512], bpath[512], mpath[512];
    snprintf(gpath,sizeof(gpath), "%s/../../output_grayscale-c.bmp", outdir);
    snprintf(bpath,sizeof(bpath), "%s/../../output_mgrad-c.bmp", outdir);
    snprintf(mpath,sizeof(mpath), "%s/../../output_mgrad-c.mem", outdir);
    save_bmp_gray(gpath, gray);
    save_bmp_gray(bpath, out);
    save_mem(mpath, out);

    free(gray); free(out);
    printf("Done C: Morphological Gradient\n");
    return 0;
}
