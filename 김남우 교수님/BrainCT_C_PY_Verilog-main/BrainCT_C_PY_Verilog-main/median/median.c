...
// sort 9 values and take median (index 4)
static inline void sort9(uint8_t* a){
    #define SWAP(i,j) do{ if(a[i]>a[j]){ uint8_t t=a[i]; a[i]=a[j]; a[j]=t; } }while(0)
    ...
    SWAP(4,2); // now a[4] median
    #undef SWAP
}

static void median3x3(const uint8_t* src, uint8_t* dst){
    for(int y=0;y<H;y++){
        for(int x=0;x<W;x++){
            uint8_t w[9];
            int k=0;
...
int main(int argc, char** argv){
    ...
    snprintf(bpath,sizeof(bpath), "%s/../../output_median-c.bmp", outdir);
    snprintf(mpath,sizeof(mpath), "%s/../../output_median-c.mem", outdir);
    save_bmp_gray(gpath, gray);
    save_bmp_gray(bpath, out);
    save_mem(mpath, out);

    free(gray); free(out);
    printf("Done C: median\n");
    return 0;
}
