...
// Emboss 3x3 with bias 128
static void emboss3(const uint8_t* src, uint8_t* dst){
    for(int y=0;y<H;y++){
        for(int x=0;x<W;x++){
            int sum = 0;
            sum += -2*src[IDX(x-1,y-1)] + -1*src[IDX(x,y-1)]   + 0*src[IDX(x+1,y-1)];
            sum += -1*src[IDX(x-1,y  )] +  1*src[IDX(x,y  )]   + 1*src[IDX(x+1,y  )];
            sum +=  0*src[IDX(x-1,y+1)] +  1*src[IDX(x,y+1)]   + 2*src[IDX(x+1,y+1)];
            sum += 128;
            if(sum<0) sum=0; else if(sum>255) sum=255;
            dst[y*W+x] = (uint8_t)sum;
        }
    }
}
...
int main(int argc, char** argv){
    ...
    snprintf(bpath,sizeof(bpath), "%s/../../output_emboss-c.bmp", outdir);
    snprintf(mpath,sizeof(mpath), "%s/../../output_emboss-c.mem", outdir);
    save_bmp_gray(gpath, gray);
    save_bmp_gray(bpath, out);
    save_mem(mpath, out);

    free(gray); free(out);
    printf("Done C: emboss\n");
    return 0;
}
