...
static void gaussian3x3(const uint8_t* src, uint8_t* dst){
    const int K[3][3]={{1,2,1},{2,4,2},{1,2,1}};
    for(int y=0;y<H;y++){
        for(int x=0;x<W;x++){
            int s=0;
            for(int j=-1;j<=1;j++) for(int i=-1;i<=1;i++)
                s += src[IDX(x+i,y+j)] * K[j+1][i+1];
            s >>= 2; // /4
            dst[y*W+x]=clamp8(s);
        }
    }
...
int main(int argc, char** argv){
    ...
    gaussian3x3(gray,out);

    char gpath[512], bpath[512], mpath[512];
    snprintf(gpath,sizeof(gpath), "%s/../../output_grayscale-c.bmp", outdir);
    snprintf(bpath,sizeof(bpath), "%s/../../output_gaussian-c.bmp", outdir);
    snprintf(mpath,sizeof(mpath), "%s/../../output_gaussian-c.mem", outdir);
    save_bmp_gray(gpath, gray);
    save_bmp_gray(bpath, out);
    save_mem(mpath, out);

    free(gray); free(out);
    printf("Done C: gaussian\n");
    return 0;
}
