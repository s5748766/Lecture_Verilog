...
// 3x3 Gaussian-like blur with integer math: (sum + 8) >> 4
static void blur3_gauss(const uint8_t* src, uint8_t* blr){
    for(int y=0;y<H;y++){
        for(int x=0;x<W;x++){
            int s = 0;
            s += 1*src[IDX(x-1,y-1)] + 2*src[IDX(x,y-1)] + 1*src[IDX(x+1,y-1)];
            s += 2*src[IDX(x-1,y  )] + 4*src[IDX(x,y  )] + 2*src[IDX(x+1,y  )];
            s += 1*src[IDX(x-1,y+1)] + 2*src[IDX(x,y+1)] + 1*src[IDX(x+1,y+1)];
            blr[y*W+x] = (uint8_t)((s + 8) >> 4);
        }
    }
}

static void unsharp(const uint8_t* src, uint8_t* dst){
    uint8_t* blr = (uint8_t*)malloc(NPIX);
    if(!blr){ printf("OOM\n"); exit(1); }
    blur3_gauss(src, blr);
    for(int i=0;i<NPIX;i++){
        int mask = (int)src[i] - (int)blr[i];
        int out  = (int)src[i] + mask; // alpha=1.0
...
int main(int argc, char** argv){
    ...
    snprintf(bpath,sizeof(bpath), "%s/../../output_unsharp-c.bmp", outdir);
    snprintf(mpath,sizeof(mpath), "%s/../../output_unsharp-c.mem", outdir);
    save_bmp_gray(gpath, gray);
    save_bmp_gray(bpath, out);
    save_mem(mpath, out);

    free(gray); free(out);
    printf("Done C: unsharp\n");
    return 0;
}
