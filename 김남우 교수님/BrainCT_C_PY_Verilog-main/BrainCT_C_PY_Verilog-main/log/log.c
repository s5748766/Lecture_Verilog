...
// LoG 5x5 (zero-sum) + bias 128
static void log5(const uint8_t* src, uint8_t* dst){
    ...
    for(int y=0;y<H;y++){
        for(int x=0;x<W;x++){
            int s=0;
            for(int dy=-1; dy<=3; dy++){ /* -2..+2 */
                for(int dx=-1; dx<=3; dx++){
                    int yy = y + dy; int xx = x + dx;
                    s += K[dy+2][dx+2] * src[IDX(xx,yy)];
                }
            }
...
int main(int argc, char** argv){
    ...
    snprintf(bpath,sizeof(bpath), "%s/../../output_log-c.bmp", outdir);
    snprintf(mpath,sizeof(mpath), "%s/../../output_log-c.mem", outdir);
