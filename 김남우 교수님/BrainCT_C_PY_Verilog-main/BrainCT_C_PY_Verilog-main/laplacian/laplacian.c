...
static void laplacian3x3_4n(const uint8_t* src, uint8_t* dst){
    // 4-neighbor Laplacian with +128 bias for visualization
    for(int y=0;y<H;y++){
        for(int x=0;x<W;x++){
            int s = 0;
            s +=  4 * src[IDX(x,y)];
...
int main(int argc, char** argv){
    ...
    laplacian3x3_4n(gray,out);

    char gpath[512], bpath[512], mpath[512];
    snprintf(gpath,sizeof(gpath), "%s/../../output_grayscale-c.bmp", outdir);
    snprintf(bpath,sizeof(bpath), "%s/../../output_laplacian-c.bmp", outdir);
    snprintf(mpath,sizeof(mpath), "%s/../../output_laplacian-c.mem", outdir);
    save_bmp_gray(gpath, gray);
    save_bmp_gray(bpath, out);
    save_mem(mpath, out);

    free(gray); free(out);
    printf("Done C: laplacian\n");
    return 0;
}
