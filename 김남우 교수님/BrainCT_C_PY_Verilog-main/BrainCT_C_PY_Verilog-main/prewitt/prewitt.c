...
static void prewitt3x3(const uint8_t* src, uint8_t* dst){
    for(int y=0;y<H;y++){
        for(int x=0;x<W;x++){
            int gx = 0, gy = 0;
            // Gx
            gx += -1*src[IDX(x-1,y-1)] + 0*src[IDX(x,y-1)] + 1*src[IDX(x+1,y-1)];
            gx += -1*src[IDX(x-1,y  )] + 0*src[IDX(x,y  )] + 1*src[IDX(x+1,y  )];
            gx += -1*src[IDX(x-1,y+1)] + 0*src[IDX(x,y+1)] + 1*src[IDX(x+1,y+1)];
            // Gy
            gy +=  1*src[IDX(x-1,y-1)] + 1*src[IDX(x,y-1)] + 1*src[IDX(x+1,y-1)];
            gy +=  0*src[IDX(x-1,y  )] + 0*src[IDX(x,y  )] + 0*src[IDX(x+1,y  )];
            gy += -1*src[IDX(x-1,y+1)] - 1*src[IDX(x,y+1)] - 1*src[IDX(x+1,y+1)];
...
int main(int argc, char** argv){
    ...
    snprintf(bpath,sizeof(bpath), "%s/../../output_prewitt-c.bmp", outdir);
    snprintf(mpath,sizeof(mpath), "%s/../../output_prewitt-c.mem", outdir);
    save_bmp_gray(gpath, gray);
    save_bmp_gray(bpath, out);
    save_mem(mpath, out);

    free(gray); free(out);
    printf("Done C: prewitt\n");
    return 0;
}
