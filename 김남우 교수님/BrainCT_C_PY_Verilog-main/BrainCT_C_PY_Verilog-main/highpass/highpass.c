...
// High-pass 3x3 (no bias)
static void highpass3(const uint8_t* src, uint8_t* dst){
    for(int y=0;y<H;y++){
        for(int x=0;x<W;x++){
            int sum = 0;
            sum += -1*src[IDX(x-1,y-1)] + -1*src[IDX(x,y-1)]   + -1*src[IDX(x+1,y-1)];
            sum += -1*src[IDX(x-1,y  )] +  8*src[IDX(x,y  )]   + -1*src[IDX(x+1,y  )];
            sum += -1*src[IDX(x-1,y+1)] + -1*src[IDX(x,y+1)]   + -1*src[IDX(x+1,y+1)];
...
int main(int argc, char** argv){
    ...
    snprintf(bpath,sizeof(bpath), "%s/../../output_highpass-c.bmp", outdir);
    snprintf(mpath,sizeof(mpath), "%s/../../output_highpass-c.mem", outdir);
    save_bmp_gray(gpath, gray);
    save_bmp_gray(bpath, out);
    save_mem(mpath, out);

    free(gray); free(out);
    printf("Done C: high-pass\n");
    return 0;
}
