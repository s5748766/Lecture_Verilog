...
// Horizontal motion blur (length=9), integer truncation
static void motionblur9(const uint8_t* src, uint8_t* dst){
    for(int y=0;y<H;y++){
        for(int x=0;x<W;x++){
            int sum = 0;
            sum += src[IDX(x-4,y)] + src[IDX(x-3,y)] + src[IDX(x-2,y)] + src[IDX(x-1,y)];
            sum += src[IDX(x,y)];
            sum += src[IDX(x+1,y)] + src[IDX(x+2,y)] + src[IDX(x+3,y)] + src[IDX(x+4,y)];
            int v = sum / 9; // truncation
...
int main(int argc, char** argv){
    ...
    motionblur9(gray,out);

    char gpath[512], bpath[512], mpath[512];
    snprintf(gpath,sizeof(gpath), "%s/../../output_grayscale-c.bmp", outdir);
    snprintf(bpath,sizeof(bpath), "%s/../../output_motionblur-c.bmp", outdir);
    snprintf(mpath,sizeof(mpath), "%s/../../output_motionblur-c.mem", outdir);
    save_bmp_gray(gpath, gray);
    save_bmp_gray(bpath, out);
    save_mem(mpath, out);

    free(gray); free(out);
    printf("Done C: motion-blur\n");
    return 0;
}
