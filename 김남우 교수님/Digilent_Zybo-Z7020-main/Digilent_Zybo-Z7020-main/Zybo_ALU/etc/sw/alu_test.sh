...
REG_A=$((ALU_BASE + 0x00))
REG_B=$((ALU_BASE + 0x04))
REG_CTRL=$((ALU_BASE + 0x08))
REG_RESULT=$((ALU_BASE + 0x0C))
...
alu_compute() {
    ...
    # Write control (opcode + enable)
    local ctrl=$((opcode | 0x8))  # Set enable bit
    alu_write $REG_CTRL $ctrl
