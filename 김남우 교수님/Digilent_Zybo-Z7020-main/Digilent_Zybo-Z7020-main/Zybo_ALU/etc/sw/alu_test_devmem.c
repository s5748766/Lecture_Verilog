...
uint16_t alu_compute(uint8_t a, uint8_t b, alu_opcode_t opcode)
{
    ...
    // Set opcode and enable
    control = (opcode & 0x7) | (1 << 3);  // Enable bit at position 3
    alu_write_reg(REG_CONTROL, control);
    ...
    // Disable ALU
    alu_write_reg(REG_CONTROL, opcode & 0x7);
...
void print_registers(void)
{
    ...
    printf("  CONTROL   (0x08): 0x%08X (opcode=%u, enable=%u)\n", 
           ctrl, ctrl & 0x7, (ctrl >> 3) & 0x1);
    printf("  RESULT    (0x0C): 0x%08X (%u)\n\n", res, res & 0xFFFF);
}
